...
CDEPEND=">=dev-java/swt-${PV}:${SLOT}
	...
	>=dev-java/sat4j-core-2.1_rc2:2
	>=dev-java/sat4j-pseudo-2.1_rc2:2
...
src_prepare() {
	...
	sed -n -e '/<exec executable="tar" dir="${assemblyTempDir}">/{:A;/<\/target>/!{n;bA}}' \
			-e '/<delete dir="${assemblyTempDir}"\/>/!p' \
			-i {assemble,package}.*.xml
	eend

	# Test pde fedora patch
	epatch "${FILESDIR}/${SLOT}/eclipse-pde.build-add-package-build.patch"

	# The source build order is broken, but building the help feature earlier
	# seems to work around it. Maybe this'll be fixed in the final 3.5 release.
	sed -e '/<ant\b.*ui\.intro/i<ant antfile="build.xml" dir="../org.eclipse.help/" target="${target}"/>' \
			-i features/org.eclipse.platform/build.xml || die
}
...
src_compile() {
	...
	# Ant install misses some plugins; copy them manually
	( cd tmp/eclipse/plugins &&
		ln -s ../../../plugins/org.{apache.commons.{codec,httpclient}_*,eclipse.ecf*.jar} ./ ) \
		|| die 'failed to link missing plugins'
	...
	# P2 mistakenly uses the wrong MD5 sum for the native launcher; fix it
	local md5=$(md5sum tmp/eclipse/repo/binary/org.eclipse.launcher_eclipse_1.0.0)
	sed_xml_element 'artifact' \
			-e "/org\.eclipse\.launcher_eclipse/s/\(md5' value=\)'[0-9a-z]*'/\1'${md5%% *}'/" \
			-i tmp/eclipse/repo/artifacts.xml || die
}
